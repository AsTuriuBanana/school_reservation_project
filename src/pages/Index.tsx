...
import { DAYS_LIST, LESSON_TIMES_LIST, isRoomFreeAt, getCurrentDayName, Room } from '@/data/mockData';
...
const Index = () => {
  ...
  const filteredRooms = useMemo(() => {
    if (loading || !searchQuery) return rooms;
  ...
  }, [searchQuery, rooms, loading]);

  const floors = useMemo(() => {
    if (searchQuery || loading) return null;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-8">
        <div className="mb-8">
          <h1 className="font-heading text-3xl font-bold text-foreground mb-2">Kabinetų tvarkaraščiai</h1>
        ...
        {floors ? floors.map(([floor, floorRooms]) => (
          <div key={floor} className="mb-8 last:mb-0">
            <h2 className="font-heading text-lg font-semibold text-foreground mb-3 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">{floor}</span>
              {floor} aukštas
            </h2>

...
import { CalendarCheck, DoorOpen, MessageSquare } from 'lucide-react';
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          {freeRooms.map(room => (
            <div key={room.id} className="glass-card rounded-xl p-5 flex items-center justify-between">
              <div>
                ...
                {room.subject && (
                  <p className="text-xs font-medium text-primary mt-0.5">{room.subject}</p>
                )}
              </div>
      ...
      {/* Reservation comment dialog */}
      <Dialog open={!!reservingRoom} onOpenChange={(open) => { if (!open) setReservingRoom(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Rezervuoti kabinetą {reservingRoom}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            {selectedDay}, {selectedLesson} pamoka
          </p>
          <div className="space-y-2">
            <label className="text-sm font-medium">Komentaras (neprivaloma)</label>
            <Textarea
              placeholder="PVz.: 2B klasės testas..."
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmReserve} className="gap-1.5">
              <CalendarCheck className="h-4 w-4" />
              Rezervuoti
            </Button>
          </DialogFooter>

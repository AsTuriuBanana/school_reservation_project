...
const STORAGE_KEY = 'bulletin_messages';
...
const Bulletin = () => {
  ...
  const handlePost = () => {
    if (!newMessage.trim()) {
      toast.error('Parašykite žinutę');
      return;
    }
    const msg: BulletinMessage = {
      id: Date.now().toString(),
      author: localStorage.getItem('teacher_name') || 'Mokytojas',
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-8 max-w-2xl">
        ...
        <p className="text-muted-foreground mb-8">
          Palikite žinutę kitiems mokytojams (pvz., „312 kabinetas visą savaitę laisvas", „2B klasei 5 pamoką vyks 213 kabinete")
        </p>
        ...
        <div className="space-y-4">
          {messages.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              Kol kas nėra žinučių. Būkite pirmas!
            </div>
          )}
          {messages.map(msg => (
